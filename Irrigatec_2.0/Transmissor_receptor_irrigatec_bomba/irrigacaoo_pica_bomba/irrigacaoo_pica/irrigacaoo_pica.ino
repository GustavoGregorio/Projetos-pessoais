...
LiquidCrystal lcd(13, 3, 11, 10, 9, 8);  // Declaração do objeto tipo lcd
...
void setup()
{
  ...
  hora_irrig_periodica = minuto_irrig_periodica;
  minuto_irrig_periodica = minuto_irrig_periodica;
...
void ligar_irrig(){ 
    char *msg = "1"; // LIGADA 
    digitalWrite(saida, true); // Flash a light to show transmitting
    vw_send((uint8_t *)msg, strlen(msg));
    vw_wait_tx(); // Wait until the whole message is gone
    digitalWrite(saida, false);   
}
...
void irrigacao_noturna(){
   ...
   if((k == 1 && sinal_digital_umidade < valor_umidade) && y == 0){
    lcd.setCursor(0, 3);
    lcd.print("SITUA"); lcd.write(3); lcd.write(4); lcd.print("O: ");
    for(int i = 0;i<10;i++){
    ligar_irrig();
  ...
  else if((k == 1 && sinal_digital_umidade > valor_umidade) && y == 1){
    lcd.setCursor(0, 3);
    lcd.print("SITUA"); lcd.write(3); lcd.write(4); lcd.print("O: ");
    for(int i = 0;i<10;i++){
    desligar_irrig();
...
if((h == 1 && sinal_digital_umidade < valor_umidade) && z == 0){
    lcd.setCursor(0, 3);
    lcd.print("SITUA"); lcd.write(3); lcd.write(4); lcd.print("O: ");
    for(int i=0;i<10;i++){
    ligar_irrig();
...
if ((h == 1 && sinal_digital_umidade > valor_umidade) && z == 1){
    lcd.setCursor(0, 3);
    lcd.print("SITUA"); lcd.write(3); lcd.write(4); lcd.print("O: ");
    for(int i=0;i<10;i++){
    desligar_irrig();
